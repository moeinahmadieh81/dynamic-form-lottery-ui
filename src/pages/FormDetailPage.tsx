...
export function FormDetailPage() {
  ...
  if (formQuery.isLoading)
    return <Typography color="text.secondary">در حال دریافت فرم...</Typography>;
  if (formQuery.isError)
    return (
      <Alert severity="error">{getApiErrorMessage(formQuery.error)}</Alert>
    );
    ...
    <Stack spacing={3}>
      ...
      <Box
        sx={{
          ...
          borderColor: "divider",
          background:
            "linear-gradient(135deg, rgba(91,91,214,.08) 0%, rgba(15,157,138,.06) 100%)",
        }}
      >
        <Stack
      ...
      {success && (
        <Alert severity="success">
          فرم با موفقیت ثبت شد. ممنون از مشارکتت 🌱
        </Alert>
      )}
      {form.status === "CLOSED" && (
        <Alert severity="info">مهلت ثبت این فرم به پایان رسیده است.</Alert>
      )}
      {form.status === "DRAWN" && (
        <Alert
          severity="info"
          action={
            <Button component={RouterLink} to={`/forms/${form.id}/lottery`}>
              مشاهده نتیجه
            </Button>
          }
        >
